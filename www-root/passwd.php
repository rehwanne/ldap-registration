...
if(sizeof($errors)==0) {
        ...
        $ldapbind_passwd = @ldap_bind($ldapconni_passwd,"uid=$username,ou=users,dc=milliways,dc=info",$password);
        if($ldapbind_passwd) {
...
ldap_mod_replace()
$info["userPassword"] = "{crypt}" . crypt($password,'$6$'.generateSalt(10).'$');
